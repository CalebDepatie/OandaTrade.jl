...
function coerceTransactionDict(tDict::Dict{String,Any}) #Also used in getTransactions endpoints
    ...
    valueFields = ["price","priceBound","fullVWAP","distance","closeoutBid","closeoutAsk",
                   "marginRate","initialMarginRequired","requestedUnits","liquidity",
                   "gainQuoteHomeConversionFactor","lossQuoteHomeConversionFactor",
                   "AccountBalance","amount","financing","pl","commission","guaranteedExecutionFee",
                   "halfSpreadCost","dividend","realizedPL","bidLiquidityUsed","askLiquidityUsed"]
...
"""
    getTransactios(config::config, tID::Int)
...
function getTransactions(config::config,fromID::Int,toID::Int,type::Union{String,Nothing}=nothing)
...
function getTransactions(config::config,sinceID::Int,type::Union{String,Nothing}=nothing)
